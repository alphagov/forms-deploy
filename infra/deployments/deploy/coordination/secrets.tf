...
resource "aws_secretsmanager_secret" "chatbot_infra_notifications_sns_topic_arn" {
  #checkov:skip=CKV_AWS_149:SNS topic ARNs are not sensitive data and do not require KMS encryption
  #checkov:skip=CKV2_AWS_57:These secrets contain static SNS topic ARNs, not rotating credentials

  name        = "govuk-forms/chatbot/infra-notifications-sns-topic-arn"
  description = "ARN of the SNS topic for ChatBot infra notifications channel (managed in deploy/coordination/chatbot.tf)"

  tags = {
    Name = "chatbot-infra-notifications-sns-topic-arn"
  }
}

resource "aws_secretsmanager_secret_version" "chatbot_infra_notifications_sns_topic_arn" {
  secret_id     = aws_secretsmanager_secret.chatbot_infra_notifications_sns_topic_arn.id
  secret_string = aws_sns_topic.infra_notifications_topic.arn
}

# Resource policy to allow deploy and integration accounts to read the secret
resource "aws_secretsmanager_secret_policy" "chatbot_infra_notifications_sns_topic_arn" {
  secret_arn = aws_secretsmanager_secret.chatbot_infra_notifications_sns_topic_arn.arn

  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
        Sid    = "AllowDeployAndIntegrationAccountsToRead"
        Effect = "Allow"
        Principal = {
          AWS = [
            "arn:aws:iam::${module.other_accounts.deploy_account_id}:role/deployer-*",
            "arn:aws:iam::${module.other_accounts.integration_account_id}:role/deployer-*"
          ]
        }
        Action   = "secretsmanager:GetSecretValue"
        Resource = "*"
...
output "chatbot_infra_notifications_sns_topic_arn_secret_arn" {
  value       = aws_secretsmanager_secret.chatbot_infra_notifications_sns_topic_arn.arn
  description = "Secrets Manager secret ARN for ChatBot infra notifications SNS topic ARN"
}
