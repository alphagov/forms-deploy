...
resource "aws_cloudwatch_event_rule" "api_on_image_tag" {
  ...
  event_pattern = jsonencode({
    ...
    detail = {
      action-type = ["PUSH"]
      image-tag = [
        for pattern in var.deploy-forms-api-container.trigger_on_tag_patterns :
        { wildcard = pattern }
      ]
...
resource "aws_codepipeline" "deploy_api_container" {
  ...
  pipeline_type  = "V2"
  execution_mode = var.deploy-forms-api-container.pipeline_execution_mode
...
module "pull_forms_api_image_retag_and_push" {
  ...
  environment_variables = {
    ...
    APPLY_LATEST_TAG     = var.deploy-forms-api-container.apply_latest_tag
    CONTAINER_REGISTRY   = var.container_registry
  }
}
