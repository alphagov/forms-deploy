...
resource "aws_route53_record" "tempo_alloy_internal" {
  #checkov:skip=CKV2_AWS_23:Not applicable to alias records
  count   = var.tempo_settings.enabled ? 1 : 0
  zone_id = data.terraform_remote_state.forms_environment.outputs.private_internal_zone_id
  name    = "alloy-otlp.internal.${var.root_domain}"
  type    = "A"
...
resource "aws_route53_record" "tempo_metrics_internal" {
  #checkov:skip=CKV2_AWS_23:Not applicable to alias records
  count   = var.tempo_settings.enabled ? 1 : 0
  zone_id = data.terraform_remote_state.forms_environment.outputs.private_internal_zone_id
  name    = "tempo-metrics.internal.${var.root_domain}"
  type    = "A"
