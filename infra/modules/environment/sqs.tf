...
resource "aws_sqs_queue" "event_bridge_dlq" {
  ...
  name   = "${var.env_name}-eventbridge-dead-letter-queue"
...
data "aws_iam_policy_document" "allows_eventbridge_to_deliver_to_sqs" {
  statement {
    ...
    resources = ["arn:aws:sqs:eu-west-2:${data.aws_caller_identity.current.account_id}:${var.env_name}-eventbridge-dead-letter-queue"]
