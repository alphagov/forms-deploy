...
function build_module_dependency_tree() {
    ...
    function extract_module_sources() {
        ...
        local tf_files
        tf_files=$(grep -l 'module\s*"' "$dir"/*.tf 2>/dev/null || true)
...
for PARAM_NAME in $PARAMETERS; do
    ...
    STORED_JSON=$(aws ssm get-parameter --name "${PARAM_NAME}" --query 'Parameter.Value' --output text 2>/dev/null || echo "")
    STORED_SHA=$(echo "${STORED_JSON}" | jq -r '.sha' 2>/dev/null || echo "")
    ...
    if git diff "${STORED_SHA}..HEAD" -- "${PATHSPEC_ARRAY[@]}" >/dev/null 2>&1; then
