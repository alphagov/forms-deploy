...
day-to-day config values don't require a rebuild. Prometheus runs as its own
ECS service (`prometheus.tf`) but uses this same `tempo-poc-prometheus` ECR
repo and image - no separate build/push step is needed for it.
...
- Tempo + Grafana run as two containers in a single ECS task (`ecs.tf`),
  ...
  `localhost:3200`. Prometheus runs as its own ECS service (`prometheus.tf`),
  reached by both over the internal ALB at
  `http://prometheus.internal.<root_domain>` rather than `localhost`.
...
- Service-graph/span-metrics are derived by Tempo's `metrics_generator` and
  written to Prometheus (`docker/tempo/tempo.yaml.tmpl`), which Grafana's
  Tempo datasource links to via `serviceMap.datasourceUid`
  (`docker/grafana/datasources.yaml`). Prometheus's data is EFS-backed
  (`efs.tf`), so it survives task restarts rather than rebuilding from
  scratch each time.
- Prometheus is deployed with no rolling overlap
  (`deployment_maximum_percent = 100`, `deployment_minimum_healthy_percent =
  0`) and explicit `availability_zone_rebalancing = "DISABLED"`
  (`prometheus.tf`) - two Prometheus processes must never hold the same
  EFS-mounted TSDB directory open concurrently, so the old task is always
  fully stopped before a replacement starts. The tradeoff is a brief
  metrics-collection gap on every Prometheus redeploy (Tempo's remote-write
  and Grafana's queries just fail and retry) - acceptable for a POC.
- Grafana's UI is exposed publicly via the existing CloudFront distribution
  ...
  (`tempo-otlp.internal.<root_domain>`) and Prometheus access
  (`prometheus.internal.<root_domain>`) are internal-ALB-only, using
  OTLP/HTTP (port 4318)/plain HTTP (port 9090) rather than gRPC so both can
  use the existing plain-HTTP internal listener, the same as every other
  internal app-to-app route in this repo - a gRPC target group would need
  the internal HTTPS listener plus SNI certificate coverage, which isn't
  worth it here.
- Neither the tempo/grafana task nor the prometheus task has internet
  egress (see `security-groups.tf`), so all three images are pulled from our
  own ECR rather than Docker Hub directly - see the one-time setup above.
  This is also why several Grafana update-check / telemetry background jobs
  are explicitly disabled in `ecs.tf` (they'd otherwise just time out
  repeatedly against grafana.com).
