...
- Create passwords for the `forms-api` and `forms-admin` users and store in SSM Parameter Store as secure strings in the `/${app-name}-${environment}/database/password` paramaters that were created as part of the rds module. Database URLs will be automatically generated by Terraform.
