...
describe "terraform vars" do
   ...
   describe(file_name) do
       ...
       applications = ["forms_admin_settings", "forms_api_settings", "forms_product_page_settings", "forms_runner_settings"]
       applications.each do |app|
           describe app do
               ...
               it "must have CPU set to specified values" do
                   cpu = json[app]["cpu"]
                   expect([256, 512, 1024, 2048, 4096, 8192, 16384]).to include(cpu)
               end
               ...
               it "must check cpu and memory are compatible" do
                   ...
                   when 512
                       expect([1024, 2048, 4096, 8192, 16384]).to include(memory)
                   when 1024
